        ...
        <div class="card-body">
          ...
          <form method="POST" name="formregistrasi" action="{{ route('registerasmahasiswa') }}">
            ...
            <form method="POST" name="formregistrasi" action="{{ route('register') }}">
              ...
              <div class="form-group row">
                @if(isset($asmahasiswa))
                <label for="nik" class="col-md-4 col-form-label text-md-right">{{ __('NIM') }}</label>
                @else
              ...
              <div class="form-group row">
                ...
                <div class="col-md-6">
                  <input id="email" type="email" aria-describedby="emailhelp" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ old('email') }}" required autocomplete="email">
                  <small id="emailhelp" class="form-text text-muted text-left">
                    Pastikan susunan huruf email benar dan email tersinkronasi ke smartphone. (email akan digunakan sebagai sarana notifikasi)
                  </small>
